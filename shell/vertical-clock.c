...
static void maynard_vertical_clock_init(MaynardVerticalClock *self)
{
...
static void
wall_clock_notify_cb(GnomeWallClock *wall_clock, GParamSpec *pspec, MaynardVerticalClock *self)
{
	GDateTime *datetime;
	gchar *str;
...
static void maynard_vertical_clock_constructed(GObject *object)
{
	...
	gtk_style_context_add_class(gtk_widget_get_style_context(padding), "maynard-clock");
	...
	gtk_style_context_add_class(gtk_widget_get_style_context(self->priv->label), "maynard-clock");
	gtk_label_set_justify(GTK_LABEL(self->priv->label), GTK_JUSTIFY_CENTER);
	gtk_widget_set_size_request(self->priv->label, MAYNARD_VERTICAL_CLOCK_WIDTH, -1);
	gtk_box_pack_start(GTK_BOX(self), self->priv->label, FALSE, FALSE, 0);

	wall_clock_notify_cb(self->priv->wall_clock, NULL, self);
}

static void maynard_vertical_clock_dispose(GObject *object)
{
	MaynardVerticalClock *self = MAYNARD_VERTICAL_CLOCK(object);
...
static void maynard_vertical_clock_class_init(MaynardVerticalClockClass *klass)
{
...
GtkWidget *maynard_vertical_clock_new()
{
	return g_object_new(MAYNARD_VERTICAL_CLOCK_TYPE, NULL);
}
