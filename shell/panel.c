...
static void app_menu_button_clicked_cb(GtkButton *button, MaynardPanel *self)
{
	g_signal_emit(self, signals[APP_MENU_TOGGLED], 0);
}

static void system_button_clicked_cb(GtkButton *button, MaynardPanel *self)
{
	g_signal_emit(self, signals[SYSTEM_TOGGLED], 0);
}

static void volume_button_clicked_cb(GtkButton *button, MaynardPanel *self)
{
	g_signal_emit(self, signals[VOLUME_TOGGLED], 0);
}

static void favorite_launched_cb(MaynardFavorites *favorites, MaynardPanel *self)
{
	g_signal_emit(self, signals[FAVORITE_LAUNCHED], 0);
}

static void maynard_panel_constructed(GObject *object)
{
	...
	gtk_revealer_set_transition_type(GTK_REVEALER(self->priv->revealer_buttons), GTK_REVEALER_TRANSITION_TYPE_SLIDE_LEFT);
	gtk_revealer_set_reveal_child(GTK_REVEALER(self->priv->revealer_buttons), TRUE);
	gtk_box_pack_start(GTK_BOX(menu_box), self->priv->revealer_buttons, FALSE, FALSE, 0);
	...
	gtk_style_context_add_class(gtk_widget_get_style_context(button), "maynard-system");
	gtk_style_context_remove_class(gtk_widget_get_style_context(button), "button");
	gtk_style_context_remove_class(gtk_widget_get_style_context(button), "image-button");
	g_signal_connect(button, "clicked", G_CALLBACK(system_button_clicked_cb), self);
	...
	ebox = gtk_event_box_new();
	gtk_box_pack_start(GTK_BOX(buttons_box), ebox, FALSE, FALSE, 0);
	button = gtk_button_new_from_icon_name(self->priv->volume_icon_name, GTK_ICON_SIZE_LARGE_TOOLBAR);
	gtk_style_context_add_class(gtk_widget_get_style_context(button), "maynard-audio");
	gtk_style_context_remove_class(gtk_widget_get_style_context(button), "button");
	gtk_style_context_remove_class(gtk_widget_get_style_context(button), "image-button");
	g_signal_connect(button, "clicked", G_CALLBACK(volume_button_clicked_cb), self);
	gtk_container_add(GTK_CONTAINER(ebox), button);
	widget_connect_enter_signal(self, ebox);
	...
	gtk_revealer_set_transition_type(GTK_REVEALER(self->priv->revealer_clock), GTK_REVEALER_TRANSITION_TYPE_SLIDE_RIGHT);
	gtk_revealer_set_reveal_child(GTK_REVEALER(self->priv->revealer_clock), FALSE);
	gtk_box_pack_start(GTK_BOX(menu_box), self->priv->revealer_clock, FALSE, FALSE, 0);

	/* vertical clock */
	gtk_container_add(GTK_CONTAINER(self->priv->revealer_clock), maynard_vertical_clock_new());
	...
	g_signal_connect(favorites, "app-launched", G_CALLBACK(favorite_launched_cb), self);

	/* bottom app menu button */
	ebox = gtk_event_box_new();
	gtk_box_pack_end(GTK_BOX(main_box), ebox, FALSE, FALSE, 0);
	button = maynard_app_icon_new("view-grid-symbolic");
	g_signal_connect(button, "clicked", G_CALLBACK(app_menu_button_clicked_cb), self);
	gtk_container_add(GTK_CONTAINER(ebox), button);
	widget_connect_enter_signal(self, ebox);
...
static void maynard_panel_dispose(GObject *object)
{
	MaynardPanel *self = MAYNARD_PANEL(object);
...
static void maynard_panel_class_init(MaynardPanelClass *klass)
{
...
static void set_icon(GtkWidget *button, const gchar *icon_name)
{
...
void maynard_panel_set_volume_icon_name(MaynardPanel *self, const gchar *icon_name)
{
	g_free(self->priv->volume_icon_name);
